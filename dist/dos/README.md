...
1. 해당 exe와 `gcdsd.cnf`를 DOS로 복사하고 `token` 수정(**DOS는 8.3이라 `.cnf`만 인식**).
2. 시리얼: `gcdsd.cnf`에 `serial = COM1:9600`. 실행 `gcdsd-serial.exe`.
3. TCP: 패킷드라이버·WATTCP.CFG 준비 후 `gcdsd-tcp.exe`.
