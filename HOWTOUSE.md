...
# 2) 데몬 설정 후 실행 (다른 터미널)
cp etc/gcdsd.cnf gcdsd.conf            # token 을 원하는 값으로 수정
./gcdsd -c gcdsd.conf                  # "listening on port 9910"

# 3) 클라이언트 설정 (gcds.conf의 host.local.* 사용)
cp etc/gcds.cnf ~/.gcds.conf           # host.local.token 을 위와 동일하게
...
2. `gcdsd.cnf` → `gcdsd.conf`로 복사하고 `token`을 바꾼다.
3. 데몬 실행.

| 대상 | 복사할 것 | 실행 |
|------|-----------|------|
| Linux | `dist/linux/gcdsd` | `./gcdsd -c gcdsd.conf` |
| macOS | `dist/macos/gcdsd` | `./gcdsd -c gcdsd.conf` |
| Haiku/BeOS | `dist/haiku/gcdsd` | `./gcdsd -c gcdsd.conf` |
| NeXTSTEP | `dist/next/gcdsd` | `./gcdsd -c gcdsd.conf` |
| Windows | `dist/win32/gcdsd.exe` | `gcdsd.exe -c gcdsd.conf` |
| MS-DOS(시리얼) | `dist/dos/gcdsd-serial.exe` | `gcdsd-serial.exe -c gcdsd.conf` |
| MS-DOS(TCP) | `dist/dos/gcdsd-tcp.exe` | `gcdsd-tcp.exe -c gcdsd.conf` |

### 3-2. 플랫폼별 실행 요령
- **Windows/MSVC**: 컴파일을 시키려면 데몬을 vcvars 적용 환경에서 띄우거나,
  명령에 `vcvars32.bat && cl ...`처럼 접두한다(doc/win32.md). COM
  시리얼도 지원(`serial = COM1:9600`).
- **MS-DOS**: 단일 태스킹이라 **비동기 필수** — `gcdsd.conf`에
...
### 4-1. 데몬 `gcdsd.conf` (템플릿 `etc/gcdsd.cnf`)
```ini
...
```
실행: `gcdsd [-c gcdsd.conf]` (경로 생략 시 기본 동작).

### 4-2. 클라이언트 `gcds.conf` (템플릿 `etc/gcds.cnf`)
검색 순서: `$GCDS_CONF` → `./gcds.conf` → `~/.gcds.conf`.
```ini
# TCP 호스트
...
### 경로 매핑
로컬 경로를 원격 경로로 자동 변환(cwd 및 명령 인자). `gcds.conf`:
```ini
...
- **데몬**(원격): `gcdsd.conf`에 `serial = <device>:<baud>`
...
| `AUTH`/토큰 오류 | 양단 `token` 불일치. `gcdsd.conf`와 `host.<alias>.token` 확인 |
