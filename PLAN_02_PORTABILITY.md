...
- **텍스트 인코딩은 와이어에서 UTF-8(가능하면 NFC)** — common/textcv.c.
  각 데몬이 자기 환경을 이 규약으로 정규화한다:
  - **Win32**: 로컬 코드페이지(CP949/932/936/950/1252 …) ↔ UTF-8을
    `MultiByteToWideChar`/`WideCharToMultiByte`(UTF-16 경유)로 변환.
    코드페이지는 `GetConsoleOutputCP()`→`GetACP()`로 **런타임 감지**하며
    하드코딩하지 않는다. 이미 65001이면 변환을 건너뛴다. 설정
    `codepage = auto|off|<번호>`로 override 가능.
  - **macOS**: HFS+는 파일명을 NFD로 정규화하고 APFS는 보존하므로
    컴파일러가 NFD를 뱉을 수 있다. 바이트가 달라지면 문자열 비교와
    `--mapback` 경로 매칭이 어긋나므로 **한글만 NFD→NFC로 조합**한다
    (Unicode 3.12 산술식, 테이블 불필요). 라틴 결합문자는 미지원.
  - **MS-DOS / NeXTSTEP**: **ASCII 전제**. 변환하지 않으며 textcv를
    링크조차 하지 않는다(DOS는 DGROUP, NeXTSTEP은 자체 8비트 인코딩에
    한글이 없어 실사용 텍스트가 ASCII).
  - **Linux/Haiku**: 이미 UTF-8이라 그대로 통과.
  - 구현 주의: 멀티바이트 문자가 **읽기 청크 경계에서 잘릴 수 있어**
    꼬리를 다음 호출로 넘기는 hold-back이 필수이고, 스트림 끝에는
    `txt_flush()`로 잔여를 방출해야 한다. 변환은 텍스트를 최대 1.5배까지
    늘리므로 출력 버퍼에 여유가 필요하다. **D 프레임은 절대 변환 금지.**
