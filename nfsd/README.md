...
- **READDIR은 요청의 `count`를 지킨다**(RFC 1094). 응답의 디렉터리
  데이터를 `count`(RD_MAXDATA=4096로 클램프) 이내로 담고 남는 엔트리는
  cookie로 다음 호출에 넘긴다. 이를 어기면(과거엔 상수 7500B로 채웠다)
  count=4096으로 요청하는 NeXTSTEP이 큰 디렉터리 응답을 디코드하지 못해
  `RPC: Can't decode result`가 났다.
...
`gnfsd`는 **포그라운드로 동작**한다 — 시작 두 줄을 찍은 뒤 요청을
기다리며 블로킹하는 것이 정상이며, 스스로 데몬화(백그라운드로 분리)하지
않는다. 백그라운드로 돌리려면 `&`를 붙이거나 아래 `serve.sh`(기본이
백그라운드, PID 출력)를 쓴다.
...
보고된 실패는 부하로는 재현되지 않았고, **원인은 심볼릭 링크였다**
(아래). 새 옵션은 성능이 동등하면서 타임아웃 시 **실패 대신 재시도**
하므로 그 상황에서 버티게 해주는 안전한 기본값이다.

### 진짜 원인이었던 것: READLINK 미구현

`lstat`을 쓰므로 LOOKUP은 심링크를 NFLNK로 정확히 보고했지만
**READLINK(proc 5)가 없어** 클라이언트가 링크를 해석할 수 없었다.
구형 NeXTSTEP 클라이언트는 그 오류를 서버 장애로 보고
`NFS server not responding still trying`으로 무한 재시도했고,
...
READLINK(5)와 대칭을 위한 SYMLINK(13)를 구현한 뒤 실기 재검증:
...
실기 확인(OPENSTEP): 마운트를 유지한 채 gnfsd를 재시작(PID 변경 확인)한
뒤 **재마운트 없이** `ls`(17개), 심링크 `cat`(종료코드 0), 전체 순회
(2861 파일)가 모두 정상 동작했다. 구형 NeXTSTEP 클라이언트가 보관하던
핸들이 그대로 통한다.
...
- READDIR은 호출당 재스캔(O(n^2)). 측정상 4000 엔트리 순회가 15ms라
  실사용에서는 문제되지 않아 최적화하지 않았다. (응답 크기는 요청 `count`를
  지킨다 — 위 구현 범위 참조.)
